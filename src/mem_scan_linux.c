        ...
        while(!value_ok)
        {
            switch(type_index)
            {
                case OPT_1BYTE:
                    ...
                    matches = execute_scanner_STD_MTH(target, &value.byte, data_size, &n_matches);
                    break;
                ...
                case OPT_INT16:
                    ...
                    matches = execute_scanner_STD_MTH(target, value.bytes16, data_size, &n_matches);
                    break;
                ...
                case OPT_INT32:
                    ...
                    matches = execute_scanner_STD_MTH(target, value.bytes32, data_size, &n_matches);
                    break;
                ...
                case OPT_INT64:
                    ...
                    matches = execute_scanner_STD_MTH(target, value.bytes64, data_size, &n_matches);
                    break;
...
bool ask_more_scans(){
    ...
    int keep_searching = 0;
    while(!correct_choice){
        printf("Do you want to scan more values? (0: no; 1: yes): ");
        scanf("%i", &keep_searching);
        if(keep_searching < 0 || keep_searching > 1){
            printf("\nPlease, select a valid choice (1 for YES, 0 for NO): ");
        } else correct_choice = true;
